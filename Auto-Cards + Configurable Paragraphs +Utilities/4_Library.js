...
function AutoCards(inHook, inText, inStop) {
  ...
  const DEFAULT_BANNED_TITLES_LIST =
    "North, East, South, West, Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, January, February, March, April, May, June, July, August, September, October, November, December"; // (mimic this comma-list "text" format)
  ...
  const [used, bans, auto, forenames, surnames] = Array.from(
    { length: 5 },
    () => new Set()
  );
  ...
  const AC = (function () {
    ...
    data = getSingletonCard(
      false,
      O.f({ ...dataVariants.critical }),
      O.f({ ...dataVariants.debug })
    );
      ...
      validate(
        ...
        O.f({
          config: [
            ...
            "bulletedListMode",
            "defaultEntryLimit",
            "defaultCardsDoMemoryUpdates",
            "defaultMemoryLimit",
            "memoryCompressionRatio",
            "ignoreAllCapsTitles",
            "readFromInputs",
            "minimumLookBackDistance",
            "LSIv2",
            "showDebugData",
            "generationPrompt",
            "compressionPrompt",
          ...
          message: ["previous", "suppress", "pending", "event"],
          chronometer: ["turn", "step", "amnesia", "postpone"],
          database: {
            titles: [
              "used",
              "banned",
            ...
            memories: ["associations", "duplicates"],
          },
        })
      )
    ) {
    ...
    return {
      ...
      generation: {
        ...
        cooldown: validateCooldown(
          underQuarterInteger(validateCooldown(DEFAULT_CARD_CREATION_COOLDOWN))
        ),
        ...
        pending: (function () {
          if (DEFAULT_DO_AC !== false) {
            const startupMessage =
              'Enabled! You may now edit the "Configure Auto-Cards" story card';
  ...
  if (!HOOK) {
    ...
    if (HOOK === null) {
      if (/Recent\s*Story\s*:/i.test(text)) {
        ...
        text = text
          ...
          .replace(
            /\s*>>>\s*Auto-Cards\s*has\s*been\s*enabled!\s*<<<\s*/gi,
            " "
          )
          .replace(/^.*\/\s*A\s*C.*$/gim, "%@%")
          .replace(/\s*%@%\s*/g, " ")
          ...
          .replace(
            /(?:(?:\s*>>>\s*please\s*select\s*"continue"\s*\([\s\S]*?\)\s*<<<\s*)+)/gi,
            (message) => {
              ...
              return (
                ...
                "@".repeat(
                  ...
                  (
                    message.match(
                      />>>\s*please\s*select\s*"continue"\s*\([\s\S]*?\)\s*<<</gi
                    ) || []
                  ).length
                ) +
                "%"
          ...
          .replace(
            /(Memories\s*:)\s*([\s\S]*?)\s*(Recent\s*Story\s*:|$)/i,
            (_, left, memories, right) => {
              return (
                left +
                "\n" +
                memories
                  ...
                  .filter((memory) => {
                    ...
                    return !(
                      (lowerMemory.includes("select") &&
                        lowerMemory.includes("continue")) ||
                      lowerMemory.includes(">>>") ||
                      lowerMemory.includes("<<<") ||
          ...
          Object.fromEntries(
            Object.entries({
              ...
              postponeEvents: function (turns) {
                if (Number.isInteger(turns) && 0 <= turns) {
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      turns +
                      '" -> AutoCards().API.postponeEvents() must be be called with a non-negative integer'
                  );
                }
                return {
              ...
              emergencyHalt: function (shouldHalt) {
                ...
                if (
                  scopeRestriction.stack &&
                  scopeRestriction.stack.includes("isolateLSIv2")
                ) {
                  throw new Error(
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      shouldHalt +
                      '" -> AutoCards().API.emergencyHalt() must be called with a boolean true or false'
                  );
                }
                return shouldHalt;
              },
              ...
              suppressMessages: function (shouldSuppress) {
                if (typeof shouldSuppress === "boolean") {
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      shouldSuppress +
                      '" -> AutoCards().API.suppressMessages() must be called with a boolean true or false'
                  );
                }
              ...
              toggle: function (toggleType) {
                if (toggleType === undefined) {
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      toggleType +
                      '" -> AutoCards().API.toggle() must be called with either A) a boolean true or false, B) a null argument, or C) no arguments at all (undefined)'
                  );
                }
                return toggleType;
              },
              ...
              generateCard: function (request, extra1, extra2) {
                ...
                if (typeof request === "string") {
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      request +
                      '" -> AutoCards().API.generateCard() must be called with either 1, 2, or 3 strings OR a correctly formatted card generation object'
                  );
                }
                O.f(request);
              ...
              redoCard: function (request, useOldInfo = true, newInfo = "") {
                if (typeof request === "string") {
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      request +
                      '" -> AutoCards().API.redoCard() must be called with a string or correctly formatted card generation object'
                  );
                }
                if (typeof useOldInfo !== "boolean") {
                  throw new Error(
                    'Invalid argument: "' +
                      ...
                      ", " +
                      useOldInfo +
                      '" -> AutoCards().API.redoCard() requires a boolean as its second argument'
                  );
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      ...
                      ", " +
                      newInfo +
                      '" -> AutoCards().API.redoCard() requires a string for its third argument'
                  );
                }
              ...
              setCardAsAuto: function (targetCard, setOrUnset = true) {
                if (isTitleInObj(targetCard)) {
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      targetCard +
                      '" -> AutoCards().API.setCardAsAuto() must be called with a string or card object'
                  );
                }
                if (typeof setOrUnset !== "boolean") {
                  throw new Error(
                    'Invalid argument: "' +
                      ...
                      ", " +
                      setOrUnset +
                      '" -> AutoCards().API.setCardAsAuto() requires a boolean as its second argument'
                  );
                }
                ...
                card.description = removeAutoProps(
                ...
                function checkAuto() {
                  return (
                    isAuto ||
                    /{updates: (?:true|false), limit: \d+}/.test(
                      card.description
                    )
                  );
                }
              ...
              addCardMemory: function (targetCard, newMemory) {
                if (isTitleInObj(targetCard)) {
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      targetCard +
                      '" -> AutoCards().API.addCardMemory() must be called with a string or card object'
                  );
                }
                if (typeof newMemory !== "string") {
                  throw new Error(
                    'Invalid argument: "' +
                      ...
                      ", " +
                      newMemory +
                      '" -> AutoCards().API.addCardMemory() requires a string for its second argument'
                  );
                }
                newMemory = newMemory
                  .trim()
                ...
                if (
                  ...
                  card.description
                    .replace(/\s+/g, " ")
                    .toLowerCase()
                    .includes(newMemory.toLowerCase())
                ) {
                  return false;
                ...
                if (titleKey in AC.database.memories.associations) {
                  AC.database.memories.associations[titleKey][1] =
                    StringsHashed.deserialize(
                      AC.database.memories.associations[titleKey][1],
                      65536
                    )
                      .remove(newMemory)
                  ...
                  AC.database.memories.associations[titleKey] = [
                    999,
                    new StringsHashed(65536).add(newMemory).serialize(),
                  ];
                }
                return true;
              },
              ...
              setBannedTitles: function (titles) {
                ...
                if (
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      titles +
                      '" -> AutoCards().API.setBannedTitles() must be called with either a string or an array of strings'
                  );
                }
              ...
              getCard: function (predicate, getAll = false) {
                if (typeof predicate !== "function") {
                  throw new Error(
                    'Invalid argument: "' +
                      predicate +
                      '" -> AutoCards().API.getCard() must be called with a function'
                  );
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      ...
                      ", " +
                      getAll +
                      '" -> AutoCards().API.getCard() requires a boolean as its second argument'
                  );
                }
              ...
              eraseCard: function (predicate, eraseAll = false) {
                if (isTitleInObj(predicate) && storyCards.includes(predicate)) {
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      predicate +
                      '" -> AutoCards().API.eraseCard() must be called with a function or card object'
                  );
                  ...
                  throw new Error(
                    'Invalid argument: "' +
                      ...
                      ", " +
                      eraseAll +
                      '" -> AutoCards().API.eraseCard() requires a boolean as its second argument'
                  );
                } else if (eraseAll) {
    ...
    switch (HOOK) {
      ...
      case "output": {
        ...
        if (
          isDoSayStory(previousAction.type) &&
          /escape\s*emergency\s*halt/i.test(previousAction.text)
        ) {
    ...
    const factories = O.f({
      library: () => ({
        ...
        description: "// You may also continue your Library code below",
        singleton: false,
        position: 2,
      }),
      input: () => ({
        ...
        description: "// You may also continue your Input code below",
        singleton: false,
        position: 3,
      }),
      context: () => ({
        ...
        description: "// You may also continue your Context code below",
        singleton: false,
        position: 4,
      }),
      output: () => ({
        ...
        description: "// You may also continue your Output code below",
        singleton: false,
        position: 5,
      }),
      guide: () => ({
    ...
    if (AC.config.LSIv2 !== null) {
      switch (HOOK) {
        case "input": {
          ...
          const [error, newText] = isolateLSIv2(
            parseCode(libraryCards, inputCards),
            callbackLog(logCard),
            LSI_DOMAIN
          );
        ...
        case "context": {
          ...
          const [error, ...newCodomain] = (([error, newText, newStop]) => [
            error,
            newText,
            newStop === true,
          ])(
            isolateLSIv2(
              parseCode(libraryCards, contextCards),
              callbackLog(logCard),
              LSI_DOMAIN[0],
              LSI_DOMAIN[1]
            )
        ...
        case "output": {
          // AutoCards was called within the output modifier
          const [libraryCards, outputCards, guideCard, stateCard, logCard] =
            collectCards(
              templates.library,
              templates.output,
              templates.guide,
          ...
          const [error, newText] = isolateLSIv2(
            parseCode(libraryCards, outputCards),
            callbackLog(logCard),
            LSI_DOMAIN
          );
          handleError(logCard, error);
        ...
        case "initialize": {
          collectAll();
          logToCard(
            Internal.getCard((card) => card.title === templates.log.name),
            "LSIv2 startup -> Success!"
          );
          CODOMAIN.initialize(null);
          break;
        }
      ...
      function handleError(logCard, error) {
        ...
        O.f(error);
        AC.signal.upstreamError =
          "LSIv2 encountered an error during the on" +
          HOOK[0].toUpperCase() +
          HOOK.slice(1) +
          " hook";
        if (error.message) {
          ...
          AC.signal.upstreamError = cleanSpaces(
            AC.signal.upstreamError.trimEnd()
          );
        }
      ...
      function concludeLSI(guideCard, stateCard, logCard) {
        ...
        stateCard.description = limitString(
          stringifyObject(simpleState).trim(),
          999999
        ).trimEnd();
        return;
      }
    ...
    function collectAll() {
      return collectCards(
        ...Object.keys(factories).map((key) => templates[key])
      );
    }
    ...
    function collectCards(...args) {
      ...
      const collections = O.f(
        args.map(({ name, entry, description, singleton, position }) => {
          const collection = {
            template: O.f({
      ...
      for (const card of storyCards) {
        ...
        for (const collection of collections) {
          if (
            !card.title
              .toLowerCase()
              .includes(collection.template.title.toLowerCase()) &&
            !card.keys
              .toLowerCase()
              .includes(collection.template.title.toLowerCase())
          ) {
          ...
          const [extensionA, extensionB] = [card.title, card.keys].map(
            (name) => {
      ...
      for (const collection of collections) {
        ...
        function applyComment(card) {
          card.entry = card.description =
            "// You may continue writing your code here";
          return;
        }
        function constructPrimary() {
          collection.primary = constructCard(
            collection.template,
            newCardIndex()
          );
          // I like my LSIv2 cards to display in the proper order once initialized uwu
          const templateKeys = Object.keys(factories);
          const cards = templateKeys
            .map((key) =>
              O.f({
                card: Internal.getCard(
                  (card) => card.title === templates[key].name
                ),
      ...
      return O.f(
        collections.map(({ singleton, primary, auxiliaries }) => {
          if (singleton) {
            return primary;
          } else {
            return O.f({
              primary,
              auxiliaries: O.f(auxiliaries.map(({ card }) => card)),
            });
          }
        })
      );
    ...
    switch (HOOK) {
      ...
      case "context": {
        ...
        if (
          configureCard.description !== configureCardTemplate.description ||
          bansOverwritten
        ) {
          const descConfigPatterns = getConfigureCardDescription()
            ...
            .map(
              (descPattern) => new RegExp("^\\s*" + descPattern + "\\s*:", "i")
            );
          const descConfigs = configureCard.description
            .split(Words.delimiter)
            .slice(1);
          if (
            descConfigs.length === descConfigPatterns.length &&
            descConfigs.every((descConfig, index) =>
              descConfigPatterns[index].test(descConfig)
            )
          ) {
            ...
            if (AC.config.generationPrompt !== cfg) {
              notify(
                "Changes to your card generation prompt were successfully saved"
              );
            ...
            if (AC.config.compressionPrompt !== cfg) {
              notify(
                "Changes to your card memory compression prompt were successfully saved"
              );
            ...
            if (bansOverwritten) {
              ...
              0 < AC.database.titles.pendingBans.length ||
              0 < AC.database.titles.pendingUnbans.length
            ) {
              const pendingBans = AC.database.titles.pendingBans.map(
                (pair) => pair[0]
              );
              const pendingRewrites = new Set(
                lowArr([
                  ...pendingBans,
                  ...AC.database.titles.pendingUnbans.map((pair) => pair[0]),
                ])
              );
              Internal.setBannedTitles(
                [
                  ...pendingBans,
                  ...extractDescSetting(2)
                    .split(",")
                    .filter(
                      (newBan) =>
                        !pendingRewrites.has(
                          newBan.toLowerCase().replace(/\s+/, " ").trim()
                        )
                    ),
                ],
                true
            ...
            function extractDescSetting(index) {
              return descConfigs[index]
                .replace(descConfigPatterns[index], "")
                .trim();
            }
          ...
          function overrideBans() {
            Internal.setBannedTitles(
              AC.database.titles.pendingBans.map((pair) => pair[0]),
              true
            );
        ...
        if (configureCard.entry !== configureCardTemplate.entry) {
          ...
          const settings = O.f(
            (function () {
              ...
              notify(
                "Restoring all settings and prompts to their default values"
              );
          ...
          O.f(oldConfig);
          if (
            settings.deleteallautomaticstorycards === true &&
          ...
          if (
            ...
            if (cfg) {
              pinAndSortCards(configureCard);
              notify(
                "The settings config card will now be pinned near the top of your story cards list"
              );
            } else {
              ...
              notify(
                "The settings config card will no longer be pinned near the top of your story cards list"
              );
            }
          }
          if (
            parseConfig(
              "minimumturnscooldownfornewcards",
              true,
              "addCardCooldown"
            )
          ) {
            ...
            if (
              ...
              const quarterCooldown = validateCooldown(
                underQuarterInteger(AC.config.addCardCooldown)
              );
              if (
                AC.config.addCardCooldown < oldCooldown &&
                quarterCooldown < AC.generation.cooldown
              ) {
                ...
                if (oldCooldown === AC.generation.cooldown) {
                  ...
                  AC.generation.cooldown = validateCooldown(
                    boundInteger(
                      0,
                      AC.generation.cooldown + quarterCooldown,
            ...
            switch (AC.config.addCardCooldown) {
              ...
              default: {
                notify(
                  "A new card will be generated once every " +
                    AC.config.addCardCooldown +
                    " turns, but only if your story contains available titles"
                );
                break;
              }
            }
          ...
          if (
            parseConfig(
              "newcardsuseabulletedlistformat",
              false,
              "bulletedListMode"
            )
          ) {
            if (cfg) {
              notify(
                "New card entries will be generated using a bulleted list format"
              );
            } else {
              notify(
                "New card entries will be generated using a pure prose format"
              );
            }
          }
          if (
            parseConfig(
              "maximumentrylengthfornewcards",
              true,
              "defaultEntryLimit"
            )
          ) {
            ...
            notify(
              "New card entries will be limited to " +
                AC.config.defaultEntryLimit +
                " characters of generated text"
            );
          }
          if (
            parseConfig(
              "newcardsperformmemoryupdates",
              false,
              "defaultCardsDoMemoryUpdates"
            )
          ) {
            if (cfg) {
              notify(
                "Newly constructed cards will begin with memory updates enabled by default"
              );
            } else {
              notify(
                "Newly constructed cards will begin with memory updates disabled by default"
              );
            }
          }
          if (
            parseConfig(
              "cardmemorybankpreferredlength",
              true,
              "defaultMemoryLimit"
            )
          ) {
            ...
            notify(
              "Newly constructed cards will begin with their memory bank length preference set to " +
                AC.config.defaultMemoryLimit +
                " characters of text"
            );
          }
          if (
            parseConfig(
              "memorysummarycompressionratio",
              true,
              "memoryCompressionRatio"
            )
          ) {
            ...
            notify(
              "Freshly summarized card memory banks will be approximately " +
                AC.config.memoryCompressionRatio / 10 +
                "x shorter than their originals"
            );
          }
          if (
            parseConfig(
              "excludeallcapsfromtitledetection",
              false,
              "ignoreAllCapsTitles"
            )
          ) {
            if (cfg) {
              notify(
                "All-caps text will be ignored during title detection to help prevent bad cards"
              );
            } else {
              notify(
                "All-caps text may be considered during title detection processes"
              );
            }
          }
          if (
            parseConfig(
              "alsodetecttitlesfromplayerinputs",
              false,
              "readFromInputs"
            )
          ) {
            if (cfg) {
              notify(
                "Titles may be detected from player Do/Say/Story action inputs"
              );
            } else {
              notify(
                "Title detection will skip player Do/Say/Story action inputs for grammatical leniency"
              );
            }
          }
          if (
            parseConfig(
              "minimumturnsagefortitledetection",
              true,
              "minimumLookBackDistance"
            )
          ) {
            ...
            notify(
              "Titles and names mentioned in your story may become eligible for future card generation attempts once they are at least " +
                AC.config.minimumLookBackDistance +
                " actions old"
            );
          }
          ...
          if (typeof cfg === "boolean") {
            if (AC.config.LSIv2 === null) {
              if (cfg) {
                ...
                notify(
                  "Live Script Interface v2 is now embedded within your adventure!"
                );
              }
            } else {
              if (!cfg) {
                ...
                notify(
                  "Live Script Interface v2 has been removed from your adventure"
                );
              }
            }
          }
          if (
            parseConfig("logdebugdatainaseparatecard", false, "showDebugData")
          ) {
            if (data === null) {
              if (cfg) {
                notify(
                  'State may now be viewed within the "Debug Data" story card'
                );
              } else {
              ...
              notify(
                'Debug data will be shared with the "Critical Data" story card to conserve memory'
              );
            } else {
          ...
          if (
            settings.disableautocards === true &&
            AC.signal.forceToggle !== true
          ) {
            disableAutoCards();
            break;
        ...
        const nearestUnparsedAction = boundInteger(
          0,
          currentTurn - AC.config.minimumLookBackDistance
        );
        if (
          AC.signal.recheckRetryOrErase ||
          nearestUnparsedAction <= AC.database.titles.lastActionParsed
        ) {
          ...
          AC.database.titles.lastActionParsed = boundInteger(
            -1,
            nearestUnparsedAction - 8
          );
        ...
        if (
          ...
          for (
            ...
            const thisTextHash = new StringsHashed(4096)
              .add(action.text)
              .serialize();
              ...
              for (let i = 0; i < words.length; i++) {
                ...
                if (isMinorWord(word)) {
                  if (0 < incompleteTitle.length) {
                    ...
                    if (
                      2 < incompleteTitle.length &&
                      !(
                        isMinorWord(
                          incompleteTitle[incompleteTitle.length - 1]
                        ) &&
                        isMinorWord(incompleteTitle[incompleteTitle.length - 2])
                      )
                    ) {
        ...
        for (let i = AC.database.titles.candidates.length - 1; 0 <= i; i--) {
          ...
          if (isUsedOrBanned(title) || isNamed(title)) {
            ...
            if (2 < candidate.length) {
              ...
              const sortedTurns = [...new Set(candidate.slice(1))].sort(
                (a, b) => a - b
              );
        ...
        let context = TEXT;
        const titleHeaderPatternGlobal =
          /\s*{\s*titles?\s*:\s*([\s\S]*?)\s*}\s*/gi;
        ...
        const cardEvents = (function () {
          ...
          const contextMemories = (function () {
            ...
            const duplicatesHashed = StringsHashed.deserialize(
              AC.database.memories.duplicates,
              65536
            );
            ...
            if (0 < duplicateMemories.size) {
              ...
              AC.database.memories.duplicates = duplicatesHashed
                .latest(2000)
                .serialize();
            }
            return uniqueMemories;
          ...
          const theKiller = new RegExp(
            "(?:" + leftBoundary + ")the[\\s\\S]*$",
            "i"
          );
          ...
          const titleHeaderMatches = [
            ...context.matchAll(titleHeaderPatternGlobal),
          ];
        ...
        for (const card of storyCards) {
          ...
          for (const titleKey of cardEvents.keys()) {
            ...
            if (
              ...
              /{\s*updates?\s*:\s*true\s*,\s*limits?\s*:[\s\S]*?}/i.test(
                card.description
              )
            ) {
              ...
              const associationsHashed = (function () {
                if (titleKey in AC.database.memories.associations) {
                  return StringsHashed.deserialize(
                    AC.database.memories.associations[titleKey][1],
                    65536
                  );
                } else {
              ...
              for (let i = 0; i < cardEvent.pendingMemories.length; i++) {
                if (associationsHashed.has(cardEvent.pendingMemories[i])) {
                  ...
                  !oldMemories.some(
                    (oldMemory) =>
                      0.8 <
                      similarityScore(oldMemory, cardEvent.pendingMemories[i])
                  )
                ) {
              ...
              AC.database.memories.associations[titleKey][1] =
                associationsHashed.latest(3500).serialize();
            ...
            if (cardEvent.titleHeader !== "") {
              ...
              if (cardMemoriesText === "") {
                ...
                const cardMemories = cardMemoriesText
                  .split("\n")
          ...
          const [oldTitleKey, newTitleKey] = [oldTitle, newTitle].map((title) =>
            title.toLowerCase()
          );
          ...
          if (oldTitleKey in AC.database.memories.associations) {
            AC.database.memories.associations[newTitleKey] =
              AC.database.memories.associations[oldTitleKey];
          ...
          card.entry = card.entry.replace(
            oldTitleHeader,
            oldTitleHeader.replace(oldTitle, newTitle)
          );
          card.keys = buildKeys(
            card.keys.replaceAll(" " + surname, ""),
            newKey
          );
          Internal.getUsedTitles();
          function exceedsMemoryLimit() {
            return (
              (function () {
                const memoryLimitMatch = card.description.match(
                  /limits?\s*:\s*(\d+)\s*}/i
                );
                if (memoryLimitMatch) {
          ...
          function prepareMemoryCompression(titleKey) {
            AC.compression.oldMemoryBank = isolateMemories(
              extractCardMemories().text
            );
          ...
          function extractCardMemories() {
            ...
            if (memoryHeaderMatch) {
              return O.f({
                missing: false,
                text: cleanSpaces(memoryHeaderMatch[0].trim()),
              });
            } else {
        ...
        if (isPendingGeneration()) {
          ...
          0 < AC.database.titles.candidates.length
        ) {
          ...
          let selectedTitle = (function () {
            ...
            if (
              AC.database.titles.candidates.length < 16 ||
              Math.random() < 0.6667
            ) {
            ...
            return (
              AC.database.titles.candidates
                ...
                .sort((a, b) => {
                  const turnDiff =
                    b.candidate[b.candidate.length - 1] -
                    a.candidate[a.candidate.length - 1];
                  if (turnDiff === 0) {
                ...
                .filter(
                  (title) =>
                    title !== mostRelevantTitle &&
                    title !== AC.database.titles.candidates[1][0]
                )
          ...
          while (!Internal.generateCard(O.f({ title: selectedTitle }))) {
            ...
            const lowerSelectedTitle =
              formatTitle(selectedTitle).newTitle.toLowerCase();
            const index = AC.database.titles.candidates.findIndex(
              (candidate) => {
                return (
                  formatTitle(candidate[0]).newTitle.toLowerCase() ===
                  lowerSelectedTitle
                );
              }
            );
        ...
        if (shouldTrimContext()) {
          ...
          const recentStoryPattern =
            /Recent\s*Story\s*:\s*([\s\S]*?)(%@GEN@%|%@COM@%|\s\[\s*Author's\s*note\s*:|$)/i;
          ...
          if (recentStoryMatch) {
            ...
            const sentencesMinimum =
              Math.ceil(
                AC.signal.maxChars /
                  6 /
                  (boundInteger(1, context.length) /
                    boundInteger(1, sentences.length))
              ) + 1;
            do {
              ...
              AC.signal.maxChars <
              context.length - recentStory.length + sentencesJoined.length + 3
            );
          ...
          if (isRemembering && shouldTrimContext()) {
            ...
            do {
              ...
              const cardMemoriesMatch = context.match(
                /{%@MEM@%([\s\S]+?)%@MEM@%}/
              );
            ...
            (function () {
              const memoryConstruct = (function () {
                if (AC.compression.lastConstructIndex === -1) {
                  for (
                    let i = 0;
                    i < AC.compression.oldMemoryBank.length;
                    i++
                  ) {
                    ...
                    if (
                      (AC.config.memoryCompressionRatio / 10) *
                        AC.compression.responseEstimate <
                  ...
                  AC.compression.lastConstructIndex = boundInteger(
                    0,
                    AC.compression.lastConstructIndex + 1,
                    AC.compression.oldMemoryBank.length - 1
                  );
                }
        ...
        function promptGeneration() {
          ...
          context =
            ...
            (function () {
              ...
              const entryPlaceholderPattern =
                /(?:[%\$]+\s*|[%\$]*){+\s*entry\s*}+/gi;
              if (
                entryPlaceholderPattern.test(AC.generation.workpiece.prompt)
              ) {
                // Fill all %{entry} placeholders with the partial entry
                return AC.generation.workpiece.prompt.replace(
                  entryPlaceholderPattern,
                  partialEntry
                );
              } else {
                // Append the partial entry to the end of context
                return (
                  AC.generation.workpiece.prompt.trimEnd() +
                  "\n\n" +
                  partialEntry
                );
              }
        ...
        function repositionAN() {
          ...
          const authorsNotePattern =
            /\s*(\[\s*Author's\s*note\s*:[\s\S]*\])\s*/i;
        ...
        function disableAutoCards() {
          ...
          notify(
            'Disabled! Use the "Edit to enable Auto-Cards" story card to undo'
          );
          CODOMAIN.initialize(TEXT);
          return;
        }
      ...
      case "output": {
        ...
        if (0 < AC.chronometer.postpone) {
          ...
          const generationsRemaining = (function () {
            ...
            const oldSentences = splitBySentences(
              formatEntry(AC.generation.workpiece.entry)
            )
              ...
              splitBySentences(
                ...
                .filter(
                  (sentence) =>
                    ...
                    !oldSentences.some(
                      (oldSentence) =>
                        0.75 < similarityScore(oldSentence, sentence)
                    ) &&
                    // Remove repeated sentences from within entryAddition itself
                    ![...seenSentences].some(
                      (seenSentence) =>
                        0.75 < similarityScore(seenSentence, sentence)
                    ) &&
            ...
            if (
              AC.generation.workpiece.limit <
              AC.generation.workpiece.entry.length
            ) {
              let exit = false;
              ...
              if (truncatedEntry.length < 150) {
                ...
                truncatedEntry = AC.generation.workpiece.entry.replace(
                  /\s*\S+$/,
                  ""
                );
              ...
              AC.generation.workpiece.limit - 50 <=
              AC.generation.workpiece.entry.length
            ) {
              AC.generation.workpiece.entry =
                AC.generation.workpiece.entry.trimEnd();
              return 0;
            }
            ...
            function estimateRemainingGens() {
              ...
              if (AC.generation.permitted === 34) {
                AC.generation.permitted = boundInteger(
                  6,
                  Math.floor(3.5 * remaining),
                  32
                );
              }
              return remaining;
          ...
          if (generationsRemaining <= 0) {
            notify(
              '"' +
                AC.generation.workpiece.title +
                '" was successfully added to your story cards!'
            );
            constructCard(
              O.f({
                ...
                entry: (function () {
                  ...
                  for (let i = sentences.length - 1; 0 <= i; i--) {
                    const bulletedEntry = cleanSpaces(
                      titleHeader + sentences.join(" ")
                    ).trimEnd();
          ...
          const compressionsRemaining = (function () {
            ...
            if (
              AC.compression.oldMemoryBank.length - 1 <=
              AC.compression.lastConstructIndex
            ) {
              ...
              newMemory.trim() !== "" &&
              newMemory.length < buildMemoryConstruct().length
            ) {
              // Good output, preserve and then proceed onwards
              AC.compression.oldMemoryBank.splice(
                0,
                AC.compression.lastConstructIndex + 1
              );
              AC.compression.lastConstructIndex = -1;
            ...
            return (
              boundInteger(
                1,
                joinMemoryBank(AC.compression.oldMemoryBank).length
              ) / AC.compression.responseEstimate
            );
          })();
          postOutputMessage(
            textClone,
            AC.compression.completed /
              (AC.compression.completed + compressionsRemaining)
          );
          if (compressionsRemaining <= 0) {
            const card = getAutoCard(AC.compression.titleKey);
            if (card === null) {
              notify(
                'Failed to apply summarized memories for "' +
                  AC.compression.vanityTitle +
                  '" due to a missing or invalid AC card title header!'
              );
            } else {
              const memoryHeaderMatch = card.description.match(
                /(?<={\s*updates?\s*:[\s\S]*?,\s*limits?\s*:[\s\S]*?})[\s\S]*$/i
              ...
              if (memoryHeaderMatch) {
                ...
                notify(
                  'Failed to apply summarizes memories for "' +
                    AC.compression.vanityTitle +
                    '" due to a missing or invalid AC card memory header!'
                );
              }
            }
            resetCompressionProperties();
            ...
            notify(
              'Summarizing excess memories for "' +
                AC.compression.vanityTitle +
                '"'
            );
          }
    ...
    function getAutoCard(titleKey) {
      return Internal.getCard((card) =>
        card.entry.toLowerCase().startsWith("{title: " + titleKey + "}")
      );
    }
    ...
    function similarityScore(strA, strB) {
      ...
      const [cleanA, cleanB] = [strA, strB].map((str) =>
        limitString(
          str
            .replace(/[0-9\s]/g, " ")
            .trim()
    ...
    function formatEntry(partialEntry) {
      const cleanedEntry = cleanSpaces(
        partialEntry
          .replace(/^{title:[\s\S]*?}/, "")
          .replace(/[#><@*_~]/g, "")
          .trim()
    ...
    switch (HOOK) {
      ...
      case "context": {
        ...
        if (AC.signal.forceToggle) {
          ...
          if (
            extractSettings(enableCard.entry)?.enableautocards === true &&
            AC.signal.forceToggle !== false
          ) {
    ...
    function getEnableCardTemplate() {
      ...
      return O.f({
        ...
        description:
          "Perform any Do/Say/Story/Continue action within your adventure to apply this change!",
      });
    }
  }
  function hoistConst() {
    return class Const {
      ...
      constructor(...args) {
        if (args.length !== 0) {
          this.constructor.#throwError([
            [
              args.length === 1,
              "Const cannot be instantiated with a parameter",
            ],
            ["Const cannot be instantiated with parameters"],
          ]);
        } else {
          O.f(this);
          return this;
      ...
      declare(...args) {
        if (args.length !== 0) {
          this.constructor.#throwError([
            [
              args.length === 1,
              "Instances of Const cannot be declared with a parameter",
            ],
            ["Instances of Const cannot be declared with parameters"],
          ]);
          ...
          this.constructor.#throwError(
            "Instances of Const cannot be redeclared"
          );
        } else {
          this.constructor.#throwError(
            "Instances of Const cannot be redeclared after initialization"
          );
        }
      }
      initialize(...args) {
        if (args.length !== 1) {
          this.constructor.#throwError([
            [
              args.length === 0,
              "Instances of Const cannot be initialized without a parameter",
            ],
            [
              "Instances of Const cannot be initialized with multiple parameters",
            ],
          ]);
        } else if (this.#constant === null) {
          ...
          this.constructor.#throwError(
            "Instances of Const cannot be initialized before declaration"
          );
        } else {
          this.constructor.#throwError(
            "Instances of Const cannot be reinitialized"
          );
        }
      }
      read(...args) {
        if (args.length !== 0) {
          this.constructor.#throwError([
            [
              args.length === 1,
              "Instances of Const cannot be read with a parameter",
            ],
            ["Instances of Const cannot read with any parameters"],
          ]);
        } else if (Array.isArray(this.#constant)) {
          return this.#constant[0];
        } else if (this.#constant === null) {
          this.constructor.#throwError(
            "Despite prior declaration, instances of Const cannot be read before initialization"
          );
        } else {
          this.constructor.#throwError(
            "Instances of Const cannot be read before initialization"
          );
        }
      }
          ...
          new Error(
            (function () {
              ...
              if (Array.isArray(conditionalMessagesTable)) {
                const chosenPair = conditionalMessagesTable.find(function (
                  ...args
                ) {
                  const [pair] = args;
                  const codomain = new Const().declare();
                  if (Array.isArray(pair)) {
                    if (pair.length === 1 && typeof pair[0] === "string") {
                      ...
                      Const.#throwError(
                        "Const.#throwError encountered an invalid array element of conditionalMessagesTable"
                      );
                    }
                  } else {
                    Const.#throwError(
                      "Const.#throwError encountered a non-array element within conditionalMessagesTable"
                    );
                  }
                ...
                if (Array.isArray(chosenPair)) {
                  ...
                  codomain.initialize(
                    "Const.#throwError was not called with any true conditions"
                  );
                }
                ...
                codomain.initialize(
                  "Const.#throwError could not parse the given argument"
                );
              }
              return codomain.read();
            })()
        ...
        if (error.stack) {
          codomain.initialize(
            error.stack
              ...
              .replace(
                /Error:|at\s*(?:#throwError|Const.(?:declare|initialize|read)|new\s*Const)\s*\(\d+:\d+\)/gi,
                ""
              )
              .replace(
                /AutoCards\s*\((\d+):(\d+)\)\s*at\s*<isolated-vm>:\d+:\d+\s*$/i,
                "AutoCards ($1:$2)"
              )
              .trim()
              .replace(/\s+/g, " ")
    ...
    return class Words {
      ...
      static {
        ...
        const wordListInitializers = {
          ...
          // Unwanted values
          undesirables: () => [
            [343332, 451737, 323433, 377817],
            [436425, 356928, 363825, 444048],
            [323433, 428868, 310497, 413952],
            [350097, 66825, 436425, 413952, 406593, 444048],
            [316932, 330000, 436425, 392073],
            [444048, 356928, 323433],
            [451737, 444048, 363825],
            [330000, 310497, 392073, 399300],
          ],
          delimiter: () => "——————————————————————————",
          // Source code location
          copy: () => [
            126852, 33792, 211200, 384912, 336633, 310497, 436425, 336633,
            33792, 459492, 363825, 436425, 363825, 444048, 33792, 392073,
            483153, 33792, 139425, 175857, 33792, 152592, 451737, 399300,
            350097, 336633, 406593, 399300, 33792, 413952, 428868, 406593,
            343332, 363825, 384912, 336633, 33792, 135168, 190608, 336633,
            467313, 330000, 190608, 336633, 310497, 356928, 33792, 310497,
            399300, 330000, 33792, 428868, 336633, 310497, 330000, 33792,
            392073, 483153, 33792, 316932, 363825, 406593, 33792, 343332,
            406593, 428868, 33792, 436425, 363825, 392073, 413952, 384912,
            336633, 33792, 363825, 399300, 436425, 444048, 428868, 451737,
            323433, 444048, 363825, 406593, 399300, 436425, 33792, 406593,
            399300, 33792, 310497, 330000, 330000, 363825, 399300, 350097,
            33792, 139425, 451737, 444048, 406593, 66825, 148137, 310497,
            428868, 330000, 436425, 33792, 444048, 406593, 33792, 483153,
            406593, 451737, 428868, 33792, 436425, 323433, 336633, 399300,
            310497, 428868, 363825, 406593, 436425, 35937, 33792, 3355672848,
            139592360193, 3300, 3300, 356928, 444048, 444048, 413952, 436425,
            111012, 72897, 72897, 413952, 384912, 310497, 483153, 69828, 310497,
            363825, 330000, 451737, 399300, 350097, 336633, 406593, 399300,
            69828, 323433, 406593, 392073, 72897, 413952, 428868, 406593,
            343332, 363825, 384912, 336633, 72897, 190608, 336633, 467313,
            330000, 190608, 336633, 310497, 356928, 3300, 3300, 126852, 33792,
            139425, 451737, 444048, 406593, 66825, 148137, 310497, 428868,
            330000, 436425, 33792, 459492, 79233, 69828, 76032, 69828, 76032,
            33792, 363825, 436425, 33792, 310497, 399300, 33792, 406593, 413952,
            336633, 399300, 66825, 436425, 406593, 451737, 428868, 323433,
            336633, 33792, 436425, 323433, 428868, 363825, 413952, 444048,
            33792, 343332, 406593, 428868, 33792, 139425, 175857, 33792, 152592,
            451737, 399300, 350097, 336633, 406593, 399300, 33792, 392073,
            310497, 330000, 336633, 33792, 316932, 483153, 33792, 190608,
            336633, 467313, 330000, 190608, 336633, 310497, 356928, 69828,
            33792, 261393, 406593, 451737, 33792, 356928, 310497, 459492,
            336633, 33792, 392073, 483153, 33792, 343332, 451737, 384912,
            384912, 33792, 413952, 336633, 428868, 392073, 363825, 436425,
            436425, 363825, 406593, 399300, 33792, 444048, 406593, 33792,
            451737, 436425, 336633, 33792, 139425, 451737, 444048, 406593,
            66825, 148137, 310497, 428868, 330000, 436425, 33792, 467313,
            363825, 444048, 356928, 363825, 399300, 33792, 483153, 406593,
            451737, 428868, 33792, 413952, 336633, 428868, 436425, 406593,
            399300, 310497, 384912, 33792, 406593, 428868, 33792, 413952,
            451737, 316932, 384912, 363825, 436425, 356928, 336633, 330000,
            33792, 436425, 323433, 336633, 399300, 310497, 428868, 363825,
            406593, 436425, 35937, 3300, 126852, 33792, 261393, 406593, 451737,
            50193, 428868, 336633, 33792, 310497, 384912, 436425, 406593, 33792,
            467313, 336633, 384912, 323433, 406593, 392073, 336633, 33792,
            444048, 406593, 33792, 336633, 330000, 363825, 444048, 33792,
            444048, 356928, 336633, 33792, 139425, 175857, 33792, 413952,
            428868, 406593, 392073, 413952, 444048, 436425, 33792, 310497,
            399300, 330000, 33792, 444048, 363825, 444048, 384912, 336633,
            33792, 336633, 475200, 323433, 384912, 451737, 436425, 363825,
            406593, 399300, 436425, 33792, 413952, 428868, 406593, 459492,
            363825, 330000, 336633, 330000, 33792, 316932, 336633, 384912,
            406593, 467313, 69828, 33792, 175857, 33792, 436425, 363825, 399300,
            323433, 336633, 428868, 336633, 384912, 483153, 33792, 356928,
            406593, 413952, 336633, 33792, 483153, 406593, 451737, 33792,
            336633, 399300, 370788, 406593, 483153, 33792, 483153, 406593,
            451737, 428868, 33792, 310497, 330000, 459492, 336633, 399300,
            444048, 451737, 428868, 336633, 436425, 35937, 33792, 101128769412,
            106046468352, 3300,
          ],
  ...
  function hoistInternal() {
    return class Internal {
      ...
      static generateCard(request, predefinedPair = ["", ""]) {
        ...
        if (
          title === "" ||
          ("title" in AC.generation.workpiece &&
            title === AC.generation.workpiece.title) ||
          (isAwaitingGeneration() &&
            AC.generation.pending.some(
              (pendingWorkpiece) =>
          ...
          logEvent(
            "The title '" +
              request.title +
              "' is invalid or unavailable for card generation",
            true
          );
          return false;
        }
        AC.generation.pending.push(
          O.s({
            title: title,
            type: limitString(
              (request.type || AC.config.defaultCardType).toString().trim(),
              100
            ),
            keys:
              predefinedPair[1] ||
              buildKeys(
                (request.keysStart ?? "").toString(),
                titleKeyPair.newKey
              ),
            entry: limitString(
              "{title: " +
            ...
            description: limitString(
              (function () {
                ...
                validateMemoryLimit(
                  parseInt(
                    request.memoryLimit || AC.config.defaultMemoryLimit,
                    10
                  )
                ) +
                "}" +
                (function () {
                  const cardMemoryBank = cleanSpaces(
                    (request.memoryStart ?? "").toString().trim()
                  );
                  if (cardMemoryBank === "") {
                    return "";
                  } else {
            ...
            prompt: (function () {
              let prompt = insertTitle(
                (request.entryPrompt ?? "").toString().trim() ||
                  AC.config.generationPrompt.trim(),
                title
              );
              let promptDetails = insertTitle(
                cleanSpaces(
                  (request.entryPromptDetails ?? "").toString().trim()
                ),
                title
              );
              if (promptDetails !== "") {
                const spacesPrecedingTerminalEntryPlaceholder = (function () {
                  const terminalEntryPlaceholderPattern =
                    /(?:[%\$]+\s*|[%\$]*){+\s*entry\s*}+$/i;
                ...
                switch (prompt[prompt.length - 1]) {
                  case "]": {
                    encapsulateBothPrompts("[", true, "]");
                    break;
                  }
                  case ">": {
                    encapsulateBothPrompts(null, false, ">");
                    break;
                  }
                  case "}": {
                    encapsulateBothPrompts("{", true, "}");
                ...
                prompt +=
                  ...
                  (function () {
                    if (spacesPrecedingTerminalEntryPlaceholder !== "") {
                      ...
                      return (
                        spacesPrecedingTerminalEntryPlaceholder + "%{entry}"
                      );
                    }
                    return "";
                  })();
                function encapsulateBothPrompts(
                  leftSymbol,
                  slicesAtMiddle,
                  rightSymbol
                ) {
            ...
            limit: validateEntryLimit(
              parseInt(request.entryLimit || AC.config.defaultEntryLimit, 10)
            ),
          })
        );
      ...
      static redoCard(request, useOldInfo, newInfo) {
        ...
        if (!eraseCard(card)) {
          ...
          request.entryPromptDetails =
            (request.entryPromptDetails ?? "").toString() + "\n" + newInfo;
        }
        O.f(request);
        Internal.getUsedTitles(true);
        if (
          !Internal.generateCard(request) &&
          !Internal.generateCard(request, [
            oldCard.entry.match(/^{title: ([\s\S]*?)}/)?.[1] ||
      ...
      static debugLog(...args) {
        ...
        const card = getSingletonCard(
          ...
          O.f({
            ...
            keys: debugCardName,
            entry:
              "The debug console log will print to the notes section below.",
      ...
      static getUsedTitles(isExternal = false) {
        ...
        for (const card of storyCards) {
          ...
          if (
            checkPlurals(denumberName(card.title.replace("\n", "")), (t) =>
              isBanned(t)
            )
          ) {
            checkRemaining();
            ...
            if (
              2 < cleanKeys.length &&
              checkPlurals(cleanKeys, (t) => isBanned(t))
            ) {
              checkRemaining();
              continue;
            }
          ...
          const traits = [card.entry, card.description]
            .map((str, i) => {
              // Absolute abomination uwu
              const hasUpdates =
                /updates?\s*:[\s\S]*?(?:(?:title|limit)s?\s*:|})/i.test(str);
              const hasLimit =
                /limits?\s*:[\s\S]*?(?:(?:title|update)s?\s*:|})/i.test(str);
              return [
                (function () {
                  if (hasUpdates || hasLimit) {
                    if (
                      /titles?\s*:[\s\S]*?(?:(?:limit|update)s?\s*:|})/i.test(
                        str
                      )
                    ) {
                      return 2;
                  ...
                  const inferredTitle = badTitleHeaderMatch[1]
                    .split(",")[0]
                    .trim();
                  if (
          ...
          const title = (function () {
            ...
            if (bad()) {
              ...
              if (
                !repair &&
                !(
                  card.entry === titleHeader ||
                  card.entry.startsWith(titleHeader + "\n")
                )
              ) {
                repair = true;
              }
            ...
            (function () {
              ...
              if (hasDescUpdates) {
                ...
                if (hasEntryUpdates) {
                  ...
                  1 < card.description.match(/updates?\s*:/gi)?.length
                ) {
                  repair = true;
                }
            ...
            (function () {
              ...
              if (hasDescLimit) {
                ...
                if (hasEntryLimit) {
                  ...
                  1 < card.description.match(/limits?\s*:/gi)?.length
                ) {
                  repair = true;
                }
          ...
          if (
            ...
            const pureMemories = cleanSpaces(
              memories.replace(memoryProperties, "").trim()
            );
  ...
  function getDefaultConfig() {
    ...
    return O.s({
      ...
      // Approximately how much shorter should recently compressed memories be? (ratio = 10 * old / new)
      memoryCompressionRatio: validateMemCompRatio(
      ...
      // How many (minimum) actions in the past does Auto-Cards look for named entities?
      minimumLookBackDistance: validateMinLookBackDist(
  ...
  function boundInteger(lowerBound, value, upperBound, fallback) {
    if (!Number.isInteger(value)) {
      if (!Number.isInteger(fallback)) {
        throw new Error(
          "Invalid arguments: value and fallback are not integers"
        );
      }
      value = fallback;
    }
    if (Number.isInteger(lowerBound) && value < lowerBound) {
      if (Number.isInteger(upperBound) && upperBound < lowerBound) {
        throw new Error(
          "Invalid arguments: The inequality (lowerBound <= upperBound) must be satisfied"
        );
      }
      return lowerBound;
  ...
  function getIntendedCard(targetCard) {
    ...
    const autoCard = Internal.getCard((card) =>
      card.entry.toLowerCase().startsWith("{title: " + titleKey + "}")
    );
    if (autoCard !== null) {
    ...
    return [
      Internal.getCard(
        (card) => card.title.replace(/\s+/g, " ").toLowerCase() === titleKey
      ),
      false,
      titleKey,
    ];
  ...
  function doPlayerCommands(input) {
    ...
    for (const command of (function () {
      ...
      if (!requestTitle) {
        ...
        const redo = (function () {
          const redoPattern =
            /^(?:redo|retry|rewrite|remake)[\s\.\?!:,;"'—\)\]]+\s*/i;
        ...
        if (Array.isArray(redo)) {
          ...
          for (const card of redo) {
            ...
            if (
              titleMatch &&
              Internal.redoCard(O.f({ title: titleMatch[1] }), true, "")
            ) {
              count++;
            }
          }
        ...
        function submitRequest(extra) {
          ...
          if (success) {
            const parsed =
              left +
              AC.generation.pending[AC.generation.pending.length - 1].title +
              right;
            ...
            result +=
              parsed + ' -> "' + request.title + '" is invalid or unavailable';
            logEvent(parsed);
          }
          return;
        }
  ...
  function concludeOutputBlock(templateCard) {
    if (AC.config.deleteAllAutoCards !== null) {
      ...
      if (AC.config.deleteAllAutoCards) {
        ...
        if (
          isDoSayStory(previousAction.type) &&
          /CONFIRM\s*DELETE/i.test(previousAction.text)
        ) {
          let successMessage = "Confirmation Success: ";
          const numCardsErased = Internal.eraseAllAutoCards();
          if (numCardsErased === 0) {
            successMessage +=
              "However, there were no previously generated story cards to delete!";
          } else {
  ...
  function endTurn() {
    AC.database.titles.used = [];
    AC.signal.outputReplacement = "";
    [AC.database.titles.pendingBans, AC.database.titles.pendingUnbans].map(
      (pending) => decrementAll(pending)
    );
  ...
  function logToCard(logCard, ...args) {
    ...
    if (desc.startsWith(turnDelimiter + hookDelimiterLeft)) {
      const hookDelimiterOld = desc.match(
        new RegExp(
          (
            "^" +
            turnDelimiter +
    ...
    logCard.description = limitString(
      desc
        .replace(
          header,
          header +
    ...
    if (preMessage === AC.message.previous && AC.message.pending.length !== 0) {
      ...
      if (!AC.message.suppress) {
        ...
        if (AC.message.pending.length === 1) {
          ...
          newMessage += AC.message.pending
            .map((messageLine, index) => "#" + (index + 1) + ": " + messageLine)
            .join("\n");
        }
  ...
  function promoteAmnesia() {
    ...
    if (
      AC.chronometer.amnesia++ <
      boundInteger(16, 2 * AC.config.addCardCooldown, 64)
    ) {
      return;
    }
    AC.generation.cooldown = validateCooldown(
  ...
  function extractSettings(settingsText) {
    ...
    const settingLines = settingsText
      .toLowerCase()
      .replace(/[^a-z0-9:>]+/g, "")
      .split(">");
    for (const settingLine of settingLines) {
      ...
      if (
        settingKeyValue.length !== 2 ||
        settings.hasOwnProperty(settingKeyValue[0])
      ) {
  ...
  function formatTitle(title) {
    ...
    if (
      ...
      [used, forenames, surnames].every(
        (nameset) => nameset.size === 1 && nameset.has("%@%")
      )
    ) {
    ...
    const leadingMinorWordsKiller = new RegExp(
      "^(?:" + minorWordsJoin + ")\\s",
      "i"
    );
    const trailingMinorWordsKiller = new RegExp(
      "\\s(?:" + minorWordsJoin + ")$",
      "i"
    );
    ...
    const honorAbbrevsKiller = new RegExp(
      "(?:^|\\s|-|\\/)(?:" +
        [...Words.honorifics, ...Words.abbreviations]
          .map((word) => word.replace(".", ""))
          .join("|") +
    ...
    if (
      titleWords.some(
        (word) => word === "the" || peerage.has(word.toLowerCase())
      )
    ) {
  ...
  function checkPlurals(title, predicate) {
    function check(t) {
      return t.length < 3 || 100 < t.length || predicate(t);
    }
    ...
    switch (t[t.length - 1]) {
      ...
      case "o":
        if (check(t + "es") || check(t + "s")) {
          return true;
        }
        break;
      // p>s : i -> us : Cacti -> Cactus
      case "i":
        if (check(t.slice(0, -1) + "us")) {
          return true;
        }
      // s>p : i, y -> ies : Kitty -> Kitties
      case "y":
        if (check(t.slice(0, -1) + "ies")) {
          return true;
        }
        break;
      // s>p : f -> ves : Wolf -> Wolves
      case "f":
        if (check(t.slice(0, -1) + "ves")) {
          return true;
        }
    ...
    switch (t.slice(-2)) {
      ...
      case "us":
        if (check(t.slice(0, -2) + "i")) {
          return true;
        }
        break;
      // s>p : is -> es : Thesis -> Theses
      case "is":
        if (check(t.slice(0, -2) + "es")) {
          return true;
        }
        break;
      // s>p : fe -> ves : Knife -> Knives
      case "fe":
        if (check(t.slice(0, -2) + "ves")) {
          return true;
        }
        break;
      case "sh":
  ...
  function isNamed(title, returnSurname) {
    ...
    if (
      ...
      return title
        .split(" ")
        .find(
          (casedTitleWord) => casedTitleWord.toLowerCase() === titleWords[1]
        );
    } else if (
      titleWords.length === 2 &&
  ...
  function shouldProceed() {
    return (
      AC.config.doAC && !AC.signal.emergencyHalt && AC.chronometer.postpone < 1
    );
  }
  ...
  function banTitle(title, isFinalAssignment) {
    ...
    if (!lowArr(Internal.getBannedTitles()).includes(lowerTitle)) {
      ...
      const index = AC.database.titles.pendingUnbans.findIndex(
        (pair) => pair[0].toLowerCase() === lowerTitle
      );
      if (index !== -1) {
  ...
  function unbanTitle(title) {
    ...
    if (index !== -1) {
      ...
      index = AC.database.titles.pendingBans.findIndex(
        (pair) => pair[0].toLowerCase() === lowerTitle
      );
      if (index !== -1) {
  ...
  if (state.LSIv2) {
    ...
    const memoryOverflow =
      38000 < JSON.stringify(state).length + JSON.stringify(AC).length;
    if (memoryOverflow) {
...
AutoCards(null);
function isolateLSIv2(code, log, text, stop) {
  const console = Object.freeze({ log });
  try {
    eval(code);
    return [null, text, stop];
  } catch (error) {
    return [error, text, stop];
  }
}
...
function delete_cliches(text) {
  const configTitle = "[CONFIG] Cliche Purge";
  const headerText =
    "# Put phrases in this story card to remove them.\n" +
    "# Usage: Add cliché phrases separated by commas under each category.\n" +
    "# {AGGRESSIVE:} phrases will remove the entire sentence.\n" +
    "# {PRECISE:} phrases will be removed from within the sentence.\n";

  const internalDefaults = {
    aggressive: [
      "You're really something else sometimes, you know that?",
      "scent of gunpowder",
      "Flickering candlelight dances",
      "The air is sterile and carries the scent of antiseptic.",
      "casting long",
      "sterile white walls",
      "heart monitor",
      ...
      ">",
      "cast a soft",
      "casting a sterile glow",
      "casting writhing shadows",
      "casting dark silhouettes along the walls",
      "beeping of medical",
      "casting dancing",
      "You're really something else, you know that?",
      "footsteps echoing",
      "footsteps echo",
      "echo off sterile walls",
      "boots clicking against",
      "echo of footsteps",
      "boots echoing",
      "echoing off the walls",
      "reflect the harsh glow",
      "knuckles are white",
      "fingers drumming",
      "fingers tap",
      "halo flickers",
      "halo flickering",
      "practiced care",
      "Exusiai's halo", //Yeah, it was that annoying.
      "casting harsh shadows",
      "fluorescent light",
      "skilled fingers",
      "glow casts harsh light",
      "casts a stark glow",
    ...
    precise: ["looming ominously", "exhales sharply", "hfjddfh"],
  };
  ...
  const configCard = storyCards.find((card) => card.title === configTitle);

  if (!configCard) {
    const configEntry = headerText + "\n{AGGRESSIVE:}\n\n{PRECISE:}\n";
    buildCard(configTitle, configEntry, "CONFIG", "", "", storyCards.length);
  } else {
    userDefinedConfig = parseClicheConfig(configCard.entry);
  }

  const aggressive_delete = internalDefaults.aggressive.concat(
    userDefinedConfig.aggressive
  );
  const precise_delete = internalDefaults.precise.concat(
    userDefinedConfig.precise
  );
  ...
  for (const sentence of sentences) {
    ...
    for (const phrase of aggressive_delete) {
      if (sentence.toLowerCase().includes(phrase.toLowerCase())) {
        shouldDelete = true;
        break;
      }
    }
    filtered_sentences.push(
      shouldDelete
        ? sentence.includes('"')
  ...
  let filtered_text = filtered_sentences.join("");
  ...
  const replace = [
    ["verdant", "green"],
    ["she purrs", "she whispers"],
    ["he purrs", "he whispers"],
    ["she purr", "she whisper"],
    ["he purr", "he whisper"],
    ["callous hands", "hands"],
    ["callous fingers", "fingers"],
    ["calloused hands", "hands"],
    ["calloused fingers", "hands"],
    ["fingers dancing", "fingers moving"],
    ["curtly", "shortly"],
    ["expert massage", "massage"],
    ["practiced pressure", "pressure"],
    ["expert fingers", "fingers"],
    ["skilled hands", "hands"],
    ["the skilled touch", "touch"],
    ["skilled touch", "touch"],
    ["skilled massage", "massage"],
    ["leverage", "use"],
    ["robust", "strong"],
    ["unprecedented", "new"],
    ["myriad", "many"],
  ...
  for (const [target, replacementStr] of replace) {
    const regex = new RegExp(escapeRegex(target), "gi");
    filtered_text = filtered_text.replace(regex, replacementStr);
  }
...
function parseClicheConfig(textContent) {
  const config = {
    aggressive: [],
    precise: [],
  };

  if (!textContent) return config;

  const lines = textContent.split("\n");
  let currentSection = null;

  for (const line of lines) {
    const headerMatch = line.match(/^\{(AGGRESSIVE|PRECISE):\}/i);
    if (headerMatch) {
      ...
      const phrases = line
        ...
        .map((s) => s.trim())
        .filter((s) => s);
      config[currentSection] = config[currentSection].concat(phrases);
    }
  }
...
function applyRandomReplacements(text) {
  const randomReplacementGroups = [
    ...
    {
      phrase: "move with practiced precision",
      options: ["move", "move"],
    },
  ];
  ...
  randomReplacementGroups.forEach((group) => {
    ...
    text = text.replace(regex, () => {
      const i = Math.floor(Math.random() * group.options.length);
...
function buildCard(
  ...
  if (
    ![type, title, keys, entry, description].every(
...
/**
 * AI Dungeon - No Repetitiveness (Enhanced)
 * ------------------------------------------------------------
 * This script removes repeated phrases from AI output, but only
 * triggers when the new output is sufficiently similar to one
 * of the previous outputs (default: 80%).
 *
 *

/**
 * Normalize and tokenize text for similarity checks.
 * - Lowercases
...
function tokenize(text) {
  ...
  return cleaned
    .split(" ")
    .map((w) => w.replace(/^[^\p{L}\p{N}]+|[^\p{L}\p{N}]+$/gu, "")) // trim non-letters/digits at edges
    .filter(Boolean);
}

/** Build n-grams from a token array (e.g., n = 2 for bigrams). */
...
/** Jaccard similarity between two arrays (treating them as sets). Returns 0..1. */
...
/**
 * Calculate similarity (0–100) between two texts by combining
 * unigram and bigram Jaccard similarities.
 */
function calculateSimilarityPercent(
  text1,
  text2,
  weights = { uni: 0.5, bi: 0.5 }
) {
  const t1 = tokenize(text1);
  const t2 = tokenize(text2);

  const uni = jaccardSimilarity(t1, t2);

  const bi1 = ngrams(t1, 2);
  const bi2 = ngrams(t2, 2);
  const bi = bi1.length && bi2.length ? jaccardSimilarity(bi1, bi2) : uni; // fallback to uni if too short

  const uniWeight = typeof weights.uni === "number" ? weights.uni : 0.5;
  const biWeight = typeof weights.bi === "number" ? weights.bi : 0.5;
  const sum = uniWeight + biWeight || 1;

  const score = (uni * uniWeight + bi * biWeight) / sum;
  return score * 100;
}

/**
 * Check similarity of newOutput against an array (or single string) of previous outputs.
 * Returns the max similarity and the index that matched.
 */
function maxSimilarityAgainstHistory(newOutput, previousOutputs, weights) {
  const history = Array.isArray(previousOutputs)
    ? previousOutputs
    : previousOutputs
    ? [previousOutputs]
    : [];
  ...
  for (let i = 0; i < history.length; i++) {
    const sim = calculateSimilarityPercent(newOutput, history[i], weights);
    if (sim > maxSim) {
      maxSim = sim;
      idx = i;
    }
  ...
  return { maxSimilarity: maxSim, matchedIndex: idx };
}

/**
 * Removes repeated phrases from the AI output if they also appear in the context.
 * "Repeated" here means any exact 3+ word phrase match after simple normalization.
 *
 * @param {string} ai_output
 * @param {string} context
 * @returns {string}
 */
function removeRepeatedPhrases(ai_output, context) {
  if (typeof ai_output !== "string") return "";
  if (!ai_output || typeof context !== "string" || !context) return ai_output;
  ...
  // Preserve original words (only collapse whitespace) for output splicing
  const originalOutputFlat = ai_output.replace(/\s+/g, " ").trim();
  const originalWords = originalOutputFlat ? originalOutputFlat.split(" ") : [];
  if (originalWords.length < 3) return originalOutputFlat;

  const normWords = originalWords.map((w) =>
    w.toLowerCase().replace(/[“”"‘’()[\]{}.,;:!?—\-…:]/g, "")
  );

  const ctxNorm = normalizeForMatch(context);
  if (!ctxNorm) return originalOutputFlat;

  // Helper to find phrase in context with space boundaries
  const appearsInContext = (phrase) => {
    let idx = ctxNorm.indexOf(phrase);
    while (idx !== -1) {
      const startOK = idx === 0 || ctxNorm[idx - 1] === " ";
      const endPos = idx + phrase.length;
      const endOK = endPos === ctxNorm.length || ctxNorm[endPos] === " ";
      if (startOK && endOK) return true;
      idx = ctxNorm.indexOf(phrase, idx + 1);
    }
    return false;
  };
  ...
  for (let i = 0; i <= normWords.length - 3; i++) {
    // Greedily take the longest phrase starting at i
    let foundLen = 0;
    for (let j = normWords.length; j >= i + 3; j--) {
      const phrase = normWords.slice(i, j).join(" ");
      if (!phrase) continue;
      if (appearsInContext(phrase)) {
        foundLen = j - i;
        break;
      }
    }
    if (foundLen > 0) {
      ranges.push([i, i + foundLen]); // [start, end)
      i = i + foundLen - 1; // skip past this phrase
    }
  }

  if (ranges.length === 0) return originalOutputFlat;
  ...
  // Splice out ranges from original words
